...
ByteHop 的权限不是一张“管理员 / 普通用户”表。一次请求要同时通过三层边界：
...
| 查看 Lease / Operation / Event | 仅自己的 | 全部成员 |
| 撤销 Lease、停止 Operation | 仅自己的 | 任意成员 |
| 配置 diff / reload | 否 | 是 |
...
<Callout type="warn" title="不要用宽规则再尝试排除个人">
  由于没有显式 deny，`group:data` 已经允许的 Resource 不能再通过另一条规则禁止某个组员。需要例外时，应调整组成员关系，或拆成更精确的组与 policy。
</Callout>
...
Resource 的 OpenAPI 只用于发现操作和输入结构。v0.1 不用它做运行时 method / path allowlist。通用 HTTP 与 Elasticsearch Resource 会拒绝 `CONNECT`、`TRACE` 和协议升级，但其他普通 HTTP 方法仍会转发；ClickHouse 仅允许 `GET`、`POST`。
...
完整协议行为见 [HTTP 转发边界](../reference/http-behavior)。
...
4. 用目标成员身份运行 `bytehop resources --json`；
5. 同时验证一个应允许的 Resource 和一个应拒绝的 Resource；
6. 在 Web 的 **Access**、**Sessions** 与 **Events** 确认结果。

Access 收紧会撤销已经不再满足新 policy 的 Lease。删除用户或修改密码会撤销其 Session 与派生 Lease。更多操作见[成员生命周期](./member-lifecycle)和[安全热重载](./reload)。

<Callout title="当前身份边界">
  v0.1 只支持 YAML 本地用户，还没有 OIDC、飞书登录、SCIM 或自动化用户目录同步。每位成员仍应使用独立账号；不要把共享账号当成暂时的团队便利。
</Callout>
