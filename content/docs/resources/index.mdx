...
一个 Resource 是 ByteHop 网关对外提供的最小能力单元：**一个 HTTP(S) upstream、一种资源类型、一组固定最小权限 credential，以及可选的 OpenAPI 契约。** 用户和 Agent 只看到 Resource 名称、用途与调用说明，不会收到上游地址或 credential。
...
| 组件 | 负责什么 |
| --- | --- |
| ByteHop 登录与 Access policy | 当前用户能否发现和调用这个 Resource |
| Lease | 这次访问绑定哪个用户、Resource，以及何时失效 |
| Resource credential | 网关以哪个固定身份连接上游 |
| 数据库、API 或 Bridge | 最终允许哪些表、索引、集合、租户和读写动作 |
...
ByteHop 负责“谁可领取哪个 Resource 的短期 Lease”；数据库负责表、行、配额与只读约束。职责分开后，增加新数据库类型不需要在网关里重新实现每一种权限系统。
