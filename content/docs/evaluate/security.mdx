---
title: 安全模型
description: ByteHop 保护什么、信任什么，以及哪些风险仍需部署者承担。
---

ByteHop 的目标是减少长期上游 credential 的暴露面，并让 Agent 访问可归属、可到期、可撤销。它不是“部署后自动安全”的边界产品。
...
把每个 Resource 的固定身份限制到“即使 ByteHop 被滥用，损害仍然有界”。Lease、审计和 UI 是第二层；数据库 / API 的最小权限才是最终边界。
