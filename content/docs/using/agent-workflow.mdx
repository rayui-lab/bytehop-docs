...
```mermaid
flowchart LR
  S[管理员已部署 Server] --> A[管理员创建账号与 Resource 权限]
  A --> C[用户电脑安装并登录 CLI]
  C --> K[当前 Agent 安装 Skill]
  K --> U[用户用自然语言描述任务]
  U --> R[Agent 调用获准 Resource]
```

对于普通用户，前置条件只有四项：已有 Server 地址、已有 ByteHop 账号、CLI 已安装并登录、该账号至少获准一个 Resource。Skill 位于最后一层，用来指导 Agent 调用 CLI；它不能替代前三项。

管理员配置模式是另一条路径：Agent 必须位于能够读取 Server 配置文件的受控环境，应用变更时还需要管理员登录。普通用户不需要进入配置模式。

## 安装 ByteHop Skill

先按[安装客户端](/docs/using/client-installation)安装 ByteHop CLI，并确认 `bytehop version` 可运行。Skill 不捆绑二进制，也不会自动登录、授予 Resource 或绕过 ByteHop policy。
...
Hermes 使用原生 Skill 管理器安装同一个目录：

```bash
hermes skills install rayui-lab/bytehop/skills/bytehop
```

OpenCode 或其他 Agent 使用各自的 Skill 目录或项目级 instructions，需要把完整 `bytehop/` 目录安装到该环境。安装器不会猜测未知目录，也不会为了兼容而修改多个 Agent 的配置文件。
...
这里的环境变量只针对必须还原后连接上游的数据库/API Secret。新增本地用户或改密码时，Skill 会要求管理员在可信终端运行 `bytehop hash-password --password-stdin`，再把完整 `password_hash` 写入 YAML；Agent 不应接收明文用户密码。账号哈希、用户组和 Access policy 都可以热更新。
...
这个模式面向使用已经部署好 ByteHop 的普通用户和 Agent。
